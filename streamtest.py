...
def triggertweets():
    ...
    # print(key)
    listener = StdOutListener(key, max_tweets)
...
def downloadtweets():
    ...
    si = io.StringIO()
    fieldnames = [
        'retweet_count', 'user_friends_count', 'created_at', 'user_followers_count', 'reply_count', 'name', 'location',
        'keyword', 'favorite_count', 'user_time_zone', 'tweet_hashtags', 'lang', 'user_id', 'text', 'user_description',
        'screen_name', 'retweeted', 'timestamp_ms', '_id', 'tweet_text_urls', 'url'
    ]
...
class StdOutListener(StreamListener):
    ...
    def on_data(self, data):
        ...
        # Retrieving original content from retweet
        if 'retweeted_status' in obj:
            if 'extended_tweet' in obj['retweeted_status']:
                tweet['text'] = obj['retweeted_status']['extended_tweet']['full_text']
            else:
                tweet['text'] = obj['retweeted_status']['text']
        # Retrieving full text from extended_tweet
        elif 'extended_tweet' in obj:
            tweet['text'] = obj['extended_tweet']['full_text']
